...
async def check_routing(state: LawState) -> dict:
    ...
    # Fast rule-based routing to bypass slow LLM call
    question_lower = state["question"].lower()
    needs_tax = any(kw in question_lower for kw in ["tax", "irs", "thuế", "evasion", "fbar", "fatca"])
    needs_compliance = any(kw in question_lower for kw in ["compliance", "sec", "regulation", "sox", "aml", "fcpa"])
    
    logger.info("Routing decision (fast-rules): needs_tax=%s needs_compliance=%s", needs_tax, needs_compliance)
